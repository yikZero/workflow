    ...
    if (stepFn) {
      ...
      if (!stepId) {
        return Response.json(
          {
            error: `Step function "${stepFnName}" from ${STEP_FILE} does not have a stepId property. This means the SWC client-mode transformation did not run. Check that the workflow file is being processed by the Next.js loader.`,
          },
          { status: 500 }
        );
      }
