...
async function handleBuild(args: string[]): Promise<void> {
  ...
  // Generate TypeScript wrappers for the workflow bundles. These files
  // are compiled by nest build and included via standard import (NFT traces them).
  // The WorkflowController uses these when the .mjs files aren't available.
  for (const [name, file] of [
    ['steps', 'steps.mjs'],
    ['workflows', 'workflows.mjs'],
    ['webhook', 'webhook.mjs'],
  ]) {
    try {
      const bundleContent = await readFileAsync(
        pathJoin(builder.outDir, file),
        'utf-8'
      );
      // Write as a module that re-exports the bundle's POST function
      // We encode the bundle as a data URI that can be dynamically imported
