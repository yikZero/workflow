...
async function handleBuild(args: string[]): Promise<void> {
  ...
  // Generate a TypeScript source file with the manifest as a constant.
  // This file is compiled by nest build and included via standard import,
  // which NFT always traces — solving the Vercel Lambda inclusion issue.
  ...
  try {
    ...
    await writeFileAsync(
      pathJoin(process.cwd(), 'src', '_manifest-data.ts'),
      `// Generated by workflow-nest build — do not edit\nexport const manifestJson = ${JSON.stringify(manifestContent)};\n`
    );
    console.log('[@workflow/nest] Generated src/_manifest-data.ts');
  } catch {
    // manifest may not exist (no workflows discovered)
  }

  // On Vercel, generate Build Output API with experimentalTriggers FIRST,
  // then generate TypeScript wrappers from the self-contained BOA bundles.
  if (process.env.VERCEL) {
    ...
    // Generate TypeScript wrappers from the self-contained CJS BOA bundles.
    // These are compiled by nest build and included via static imports (NFT traces them).
    const boaFuncDir = pathJoin(
      process.cwd(),
      '.vercel/output/functions/.well-known/workflow/v1'
    );
    for (const [name, funcDir] of [
      ['steps', 'step.func'],
      ['workflows', 'flow.func'],
      ['webhook', 'webhook/[token].func'],
    ]) {
      try {
        const bundleContent = await readFileAsync(
          pathJoin(boaFuncDir, funcDir, 'index.js'),
          'utf-8'
        );
        const encoded = Buffer.from(bundleContent).toString('base64');
        await writeFileAsync(
          pathJoin(process.cwd(), 'src', `_${name}-bundle.ts`),
          [
            `// Generated by workflow-nest build — do not edit`,
            `export const bundleBase64 = ${JSON.stringify(encoded)};`,
            '',
          ].join('\n')
        );
        console.log(`[@workflow/nest] Generated src/_${name}-bundle.ts`);
      } catch {
        // Bundle may not exist
      }
    }
  }
}
