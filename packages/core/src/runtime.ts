...
export function workflowEntrypoint(
  ...
  const handler = getWorldHandlers().createQueueHandler(
    ...
    async (message_, metadata) => {
      ...
      // Enter chaos request context so World implementations can apply
      // chaos behavior (e.g., routing to chaos server, injecting failures).
      // Only enter the ALS when chaos config is present to avoid returning
      // a truthy empty object from getRequestContext() in non-chaos cases.
      const runHandler = () =>
        // Invoke user workflow within the propagated trace context and baggage
        withTraceContext(traceContext, async () => {
      ...
      if (chaos || chaosSeed) {
        return await requestContext.run({ chaos, chaosSeed }, runHandler);
      }
