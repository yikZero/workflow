            ...
            async () => {
              ...
              return trace(
                ...
                async (span) => {
                  ...
                  // The snapshot runtime is a self-contained alternative to
                  // the V2 inline-replay loop below. It runs the workflow in
                  // a QuickJS VM, persists snapshots between suspensions,
                  // queues steps via the same combined route (so they hit
                  // executeStep below on re-entry), and manages its own
                  // run_completed / run_failed lifecycle. When snapshot
                  // mode is in effect, return immediately after dispatch.
                  if (useSnapshotRuntime(workflowRun)) {
                    ...
                    if (snapshotResult?.timeoutSeconds !== undefined) {
                      return { timeoutSeconds: snapshotResult.timeoutSeconds };
                    }
                    return;
                  }
