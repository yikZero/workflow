...
/**
 * V2 result that returns pending step items instead of queuing them,
 * allowing the combined handler to decide inline vs background execution.
 */
export interface SuspensionHandlerV2Result {
...
/**
 * V2 suspension handler: creates events for all pending operations (hooks, steps, waits)
 * but does NOT queue step messages. Returns the pending steps so the combined handler
 * can decide which to execute inline vs queue to background.
 *
 * Processing order is the same as V1:
 * 1. Hooks are processed first to prevent race conditions
 * 2. Step events and wait events are created in parallel
 */
export async function handleSuspensionV2({
  suspension,
  world,
  run,
  span,
}: SuspensionHandlerParams): Promise<SuspensionHandlerV2Result> {
  const runId = run.runId;
  // Separate queue items by type
  const stepItems = suspension.steps.filter(
    (item): item is StepInvocationQueueItem => item.type === 'step'
