...
// ---------------------------------------------------------------------------
// JSON-degraded Uint8Array reconstitution
// ---------------------------------------------------------------------------
 ...
 * downstream binary-format handling succeeds.
 */
function reconstituteBinary(data: unknown): unknown {
  ...
  // Quick check: all keys must be sequential numeric strings "0","1","2",...
  // and all values must be byte-range numbers (0-255)
  for (let i = 0; i < len; i++) {
    if (keys[i] !== String(i)) {
      return data;
    }
 ...
 * 2. JSON-degraded byte object `{0:100, 1:101, ...}` → reconstitute, then (1)
...
export function hydrateData(value: unknown, revivers: Revivers): unknown {
  // Reconstitute JSON-degraded byte objects before checking format
  const reconstituted = reconstituteBinary(value);

  if (reconstituted instanceof Uint8Array) {
    const { format, payload } = decodeFormatPrefix(reconstituted);
  ...
  if (Array.isArray(reconstituted)) {
    return unflatten(reconstituted, revivers);
  }
