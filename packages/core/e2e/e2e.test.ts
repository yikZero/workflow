...
describe('e2e', () => {
  ...
  test('hookWorkflow is not resumable via public webhook endpoint', {
    timeout: 60_000,
  }, async () => {
    const token = Math.random().toString(36).slice(2);
    const customData = Math.random().toString(36).slice(2);
  ...
  describe('outputStreamWorkflow - getTailIndex and getStreamChunks', () => {
    test('getTailIndex returns correct index after stream completes', {
      timeout: 60_000,
    }, async () => {
      const run = await start(await e2e('outputStreamWorkflow'), []);
      await run.returnValue;
    ...
    test('getTailIndex returns -1 before any chunks are written', {
      timeout: 60_000,
    }, async () => {
      const run = await start(await e2e('outputStreamWorkflow'), []);
    ...
    test('getStreamChunks returns same content as reading the stream', {
      timeout: 60_000,
    }, async () => {
      const run = await start(await e2e('outputStreamWorkflow'), []);
      await run.returnValue;
  ...
  test('outputStreamInsideStepWorkflow - getWritable() called inside step functions', {
    timeout: 60_000,
  }, async () => {
  ...
  describe('error handling', () => {
    describe('error propagation', () => {
      describe('workflow errors', () => {
        test('nested function calls preserve message and stack trace', {
          timeout: 60_000,
        }, async () => {
        ...
        test('cross-file imports preserve message and stack trace', {
          timeout: 60_000,
        }, async () => {
      ...
      describe('step errors', () => {
        test('basic step error preserves message and stack trace', {
          timeout: 60_000,
        }, async () => {
        ...
        test('cross-file step error preserves message and function names in stack', {
          timeout: 60_000,
        }, async () => {
    ...
    describe('retry behavior', () => {
      test('regular Error retries until success', {
        timeout: 60_000,
      }, async () => {
      ...
      test('FatalError fails immediately without retries', {
        timeout: 60_000,
      }, async () => {
      ...
      test('RetryableError respects custom retryAfter delay', {
        timeout: 60_000,
      }, async () => {
    ...
    describe('not registered', () => {
      test('WorkflowNotRegisteredError fails the run when workflow does not exist', {
        timeout: 60_000,
      }, async () => {
      ...
      test('StepNotRegisteredError fails the step but workflow can catch it', {
        timeout: 60_000,
      }, async () => {
      ...
      test('StepNotRegisteredError fails the run when not caught in workflow', {
        timeout: 60_000,
      }, async () => {
  ...
  test('stepDirectCallWorkflow - calling step functions directly outside workflow context', {
    timeout: 60_000,
  }, async () => {
  ...
  test('hookCleanupTestWorkflow - hook token reuse after workflow completion', {
    timeout: 60_000,
  }, async () => {
    const token = Math.random().toString(36).slice(2);
    const customData = Math.random().toString(36).slice(2);
  ...
  test('concurrent hook token conflict - two workflows cannot use the same hook token simultaneously', {
    timeout: 60_000,
  }, async () => {
    const token = Math.random().toString(36).slice(2);
    const customData = Math.random().toString(36).slice(2);
  ...
  test('hookDisposeTestWorkflow - hook token reuse after explicit disposal while workflow still running', {
    timeout: 90_000,
  }, async () => {
    const token = Math.random().toString(36).slice(2);
    const customData = Math.random().toString(36).slice(2);
  ...
  test('stepFunctionPassingWorkflow - step function references can be passed as arguments (without closure vars)', {
    timeout: 60_000,
  }, async () => {
  ...
  test('stepFunctionWithClosureWorkflow - step function with closure variables passed as argument', {
    timeout: 60_000,
  }, async () => {
  ...
  test('closureVariableWorkflow - nested step functions with closure variables', {
    timeout: 60_000,
  }, async () => {
  ...
  test('spawnWorkflowFromStepWorkflow - spawning a child workflow using start() inside a step', {
    timeout: 120_000,
  }, async () => {
  ...
  test('health check (queue-based) - workflow and step endpoints respond to health check messages', {
    timeout: 60_000,
  }, async () => {
  ...
  test('health check (CLI) - workflow health command reports healthy endpoints', {
    timeout: 60_000,
  }, async () => {
  ...
  test('pathsAliasWorkflow - TypeScript path aliases resolve correctly', {
    timeout: 60_000,
  }, async () => {
  ...
  test('Calculator.calculate - static workflow method using static step methods from another class', {
    timeout: 60_000,
  }, async () => {
  ...
  test('AllInOneService.processNumber - static workflow method using sibling static step methods', {
    timeout: 60_000,
  }, async () => {
  ...
  test('ChainableService.processWithThis - static step methods using `this` to reference the class', {
    timeout: 60_000,
  }, async () => {
    ...
    // - sum = 50 + 100 = 150
    const run = await start(await e2e('ChainableService.processWithThis'), [5]);
  ...
  test('thisSerializationWorkflow - step function invoked with .call() and .apply()', {
    timeout: 60_000,
  }, async () => {
  ...
  test('customSerializationWorkflow - custom class serialization with WORKFLOW_SERIALIZE/WORKFLOW_DESERIALIZE', {
    timeout: 60_000,
  }, async () => {
  ...
  test('instanceMethodStepWorkflow - instance methods with "use step" directive', {
    timeout: 60_000,
  }, async () => {
    ...
    const { json: steps } = await cliInspectJson(`steps --runId ${run.runId}`);
    ...
    expect(counterSteps.every((s: any) => s.status === 'completed')).toBe(true);
  });

  test('crossContextSerdeWorkflow - classes defined in step code are deserializable in workflow context', {
    timeout: 60_000,
  }, async () => {
  ...
  test('stepFunctionAsStartArgWorkflow - step function reference passed as start() argument', {
    timeout: 120_000,
  }, async () => {
  ...
  // ==================== CANCEL TESTS ====================
  test('cancelRun - cancelling a running workflow', {
    timeout: 60_000,
  }, async () => {
  ...
  test('cancelRun via CLI - cancelling a running workflow', {
    timeout: 60_000,
  }, async () => {
    // Start a long-running workflow with a 30s sleep to provide a wide
    // window for the cancel to arrive while the workflow is still running.
  ...
  describe.skipIf(!isNextJsApp)('pages router', () => {
    ...
    test('promiseAllWorkflow via pages router', {
      timeout: 60_000,
    }, async () => {
      const run = await startWorkflowViaHttp(
        'promiseAllWorkflow',
  ...
  test('hookWithSleepWorkflow - hook payloads delivered correctly with concurrent sleep', {
    timeout: 90_000,
  }, async () => {
  ...
  test('sleepInLoopWorkflow - sleep inside loop with steps actually delays each iteration', {
    timeout: 60_000,
  }, async () => {
  ...
  test('sleepWithSequentialStepsWorkflow - sequential steps work with concurrent sleep (control)', {
    timeout: 60_000,
  }, async () => {
    ...
    const run = await start(await e2e('sleepWithSequentialStepsWorkflow'), []);
  ...
  test('importMetaUrlWorkflow - import.meta.url is available in step bundles', {
    timeout: 60_000,
  }, async () => {
  ...
  test('metadataFromHelperWorkflow - getWorkflowMetadata/getStepMetadata work from module-level helper (#1577)', {
    timeout: 60_000,
  }, async () => {
  ...
  // ============================================================
  test('resilient start: addTenWorkflow completes when run_created returns 500', {
    timeout: 60_000,
  }, async () => {
