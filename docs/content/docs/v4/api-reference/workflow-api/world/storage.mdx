...
<Callout type="info">
`hooks.getByToken()` is useful for finding the run that owns an existing hook. It is not an atomic replacement for a run idempotency key because the hook exists only after a workflow has started and registered it. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
