...
Hook-token lookup can be used as part of an idempotency flow: if a hook is already registered for your business key, reuse the hook's `runId` instead of starting another run. It is not a fully atomic replacement for run idempotency because two callers can both miss before the first run registers the hook. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
