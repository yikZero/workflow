...
Hook-token lookup is the low-level form of the recommended idempotency flow: if a hook is already registered for your business key, reuse the hook's `runId` or resume that hook instead of starting another run. If no hook exists yet, start a workflow that creates and awaits or iterates the deterministic hook near the beginning, then handles `HookConflictError` if another run claimed the token first. Current worlds include the active owner as `conflictingRunId` on the conflict. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
