...
<Callout type="info">
For "resume or start" flows, `resumeHook()` is the resume half. If no hook exists yet and you need to call [`start()`](/docs/api-reference/workflow-api/start), protect that first start with an atomic idempotency key when duplicate runs are unsafe. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
