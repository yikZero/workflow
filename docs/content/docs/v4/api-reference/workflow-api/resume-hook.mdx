...
<Callout type="info">
For "resume or start" flows, `resumeHook()` is the resume half. If no hook exists yet, pass the payload to [`start()`](/docs/api-reference/workflow-api/start), persist it durably, or retry `resumeHook()` after the workflow has registered its deterministic hook. If you simply drop the payload and start the workflow, the data from this request is lost. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
