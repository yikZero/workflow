...
<Callout type="info">
Looking up a deterministic hook token can help route a request to an existing run. This lookup is not an atomic "start once" primitive: two callers can both miss the hook before the first run registers it. If duplicate run creation is unsafe, use an atomic reservation before [`start()`](/docs/api-reference/workflow-api/start). See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
