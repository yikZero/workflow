...
<Callout type="info">
Custom hook tokens are the recommended way to coordinate active workflow runs. Use a deterministic token from your domain, such as an order ID or conversation ID, and create the hook near the beginning of the workflow. Only one active hook can own that token; when a duplicate run creates a hook with the same token, awaiting or iterating that hook rejects with `HookConflictError`. Current worlds include `conflictingRunId` on that error so the duplicate run can route callers to the active owner. Await or iterate the hook before work that depends on owning the token. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
