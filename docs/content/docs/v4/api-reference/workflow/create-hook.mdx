...
<Callout type="info">
Custom hook tokens are the recommended way to coordinate run idempotency. Use a deterministic token from your domain, such as an order ID or conversation ID, and create the hook near the beginning of the workflow. Only one active hook can own that token; duplicate runs receive `HookConflictError` when they try to create the same hook. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
