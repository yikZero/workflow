...
### Waiting for Hook Registration

Sometimes you need to know that a hook token has been claimed, but you do not want to wait for external data yet. Await `hook.ready` for that:
...
`hook.ready` resolves when the hook is registered and ready to receive payloads. If another active hook already owns the same token, it rejects with [`HookConflictError`](/docs/errors/hook-conflict).
