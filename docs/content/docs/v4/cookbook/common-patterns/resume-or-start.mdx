...
- **Add an outside-workflow idempotency layer** — for example a database unique constraint on **your stable application ID** (the same value you embed in the hook token), a single-row transaction, or (when available on your stack) an idempotency key on **`start()`** so only one run is created per ID.
