...
<Callout type="info">
Each background spawn creates a separate run. If a parent replay or retried request must not create duplicate child runs for the same business key, use an atomic idempotency reservation around the `start()` call. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
