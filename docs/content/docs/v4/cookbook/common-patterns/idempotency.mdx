...
Use a deterministic hook token as the workflow's idempotency key. The workflow creates the hook near the beginning of the run, and external callers use that same token to find or resume the active run.
...
Hook creation is the atomic part of this flow. `start()` and the first `createHook()` call are not a single transaction today, so the caller waits until the hook is visible. If two callers both start a run before either hook is visible, only one run can own the token and the duplicate run receives `HookConflictError`.
