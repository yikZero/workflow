...
<Callout type="info">
Deterministic hook tokens route cancellation messages to the active workflow after it has started. They are not a substitute for run idempotency. If duplicate schedule starts would send duplicate campaigns or reminders, reserve the campaign key before calling `start()`. See [Idempotency](/docs/foundations/idempotency).
</Callout>
