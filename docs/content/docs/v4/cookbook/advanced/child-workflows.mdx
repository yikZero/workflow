...
<Callout type="info">
`start()` creates a new child run each time it is called. For batches that may be retried or resumed from an external request, reserve a stable business key for each child before spawning it if duplicate child runs would be unsafe. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
...
- **Use idempotency keys for duplicate-sensitive children.** If one item must map to one child run, reserve that item key atomically before calling `start()`.
...
- [Idempotency](/docs/foundations/idempotency) -- reserve business keys before duplicate-sensitive starts
