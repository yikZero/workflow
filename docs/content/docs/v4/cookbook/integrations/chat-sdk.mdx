...
<Callout type="info">
If the platform can deliver the same first message concurrently, make the `thread.setState({ runId })` path atomic or reserve the thread ID before calling `start()`. Otherwise two handlers can both see "no run yet" and start duplicate sessions. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
