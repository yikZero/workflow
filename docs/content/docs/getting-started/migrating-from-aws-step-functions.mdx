...
| Child state machine (`StartExecution`) | `start()` + `getRun()` | Spawn another workflow and poll or await its result. |
...
- **Pay for active CPU time.** When a workflow is waiting on a hook, a sleep, or an external call, you are not billed. There is no idle Lambda or Express state machine cost.
...
- Replace Map states with loops or `Promise.all()` over arrays, using child workflows via `start()` for large fan-outs.
