...
Deploying a NestJS workflow app to [Vercel](https://vercel.com/home) requires an additional build step to generate the [Build Output API](https://vercel.com/docs/build-output-api/v3).

<FluidComputeCallout />

### 1. Create the Vercel entry point

Create an `api/index.js` file that exports your NestJS app as a serverless function handler:

{/*@skip-typecheck - Shows Vercel entry point*/}

```javascript title="api/index.js" lineNumbers
...
### 2. Add the Vercel build step

Update your build script to call `buildVercelOutput()` when deploying to Vercel. This generates the Build Output API with workflow functions that have the required queue triggers:

{/*@skip-typecheck - Shows build script*/}

```typescript title="scripts/build-vercel.ts" lineNumbers
...
// Build workflow bundles for the NestJS controller
await builder.build();

// Generate Vercel Build Output API with queue triggers
...
```json title="package.json" lineNumbers
{
  "scripts": {
    "prebuild": "npx @workflow/nest init --force",
    "build": "nest build",
    "postbuild": "tsx scripts/build-vercel.ts",
    "start:dev": "npx @workflow/nest init --force && nest start --watch"
  }
}
```
...
Since `buildVercelOutput()` generates the full Build Output API structure at `.vercel/output/`, your `vercel.json` only needs the build command:
...
Do **not** add `functions`, `outputDirectory`, or `rewrites` to `vercel.json` — these are handled by the generated Build Output API `config.json`.
</Callout>
