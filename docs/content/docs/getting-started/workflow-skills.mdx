...
After copying, you should see the three core skill directories — `workflow`
(always-on reference), `workflow-teach` (stage 1), and `workflow-build`
(stage 2) — plus the optional `workflow-init` helper for first-time
project setup before `workflow` is installed as a dependency.
