...
description: Move a Temporal TypeScript workflow to Vercel Workflow by replacing Activities, Workers, Signals, and Child Workflows with Workflows, Steps, Hooks, and start()/getRun().
type: guide
summary: Translate a Temporal app into Vercel Workflow with side-by-side code and a realistic order-processing saga.
prerequisites:
...
With Vercel Workflow, the runtime is managed for you. You write `"use workflow"` functions that orchestrate `"use step"` functions — all in the same file, all plain TypeScript. There are no Workers to poll, no Task Queues to configure, and no separate Activity modules to maintain. Durable replay, automatic retries, and event history still exist — they just happen behind the scenes on Vercel's infrastructure.

The migration path is mostly about **removing infrastructure** and **collapsing indirection**, not rewriting business logic.

## Concept mapping

| Temporal | Vercel Workflow | Migration note |
| --- | --- | --- |
...
| Worker + Task Queue | Vercel-managed execution on Vercel World | No worker fleet or polling loop to operate on Vercel. |
...
| Child Workflow | `start()` + `getRun()` | Spawn another workflow and poll or await its result. |
...
- **No separate server.** Durable state lives in the Vercel-managed event log. There is no cluster to provision, monitor, or upgrade.
...
- **Pay for active CPU time.** When a workflow is waiting on a hook, a sleep, or an external call, you are not billed. There is no idle worker fleet consuming resources.
...
- Replace Child Workflows with `start()` + `getRun()` when you need independent runs.
