...
| `step.invoke()` | `start()` + `getRun()` | Spawn another workflow and await its result. |
...
| `step.sendEvent()` | `start()` inside a workflow | Fan out to other workflows with `start()`. |
...
Inngest's `step.waitForEvent()` with event matching becomes a single `createHook()` call. The workflow suspends durably until the hook is resumed — no event bus, no matching expression, and no separate timeout to configure.
  ...
  async ({ event, step }) => {
    ...
    try {
      payment = await step.run('charge-payment', async () => {
        ...
        return res.json() as Promise<{ chargeId: string }>;
      });
    } catch (error) {
    ...
    // ... shipment, more compensation, etc.
    return { orderId: order.id, status: 'completed' };
  }
);
```
...
- **Pay for active CPU time.** When a workflow is waiting on a hook, a sleep, or an external call, you are not billed. There is no separate platform metering.
...
- Replace `step.invoke()` with `start()` + `getRun()` when you need to call another workflow and await its result.
