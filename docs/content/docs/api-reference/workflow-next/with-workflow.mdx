...
|---|---|---|
| `workflows.lazyDiscovery` | `boolean` | Enable lazy discovery mode. When `true`, workflows are discovered incrementally via loader notifications instead of eagerly scanning at build time. |
| `workflows.local.port` | `number` | Override the local development server port. Sets the `PORT` environment variable. |
| `workflows.local.dataDir` | `string` | Override the local data directory path. Defaults to `'.next/workflow-data'` when running locally. |
...
- Sets `WORKFLOW_TARGET_WORLD` to `'local'` if not already set
- Sets `WORKFLOW_LOCAL_DATA_DIR` to `'.next/workflow-data'` if not already set

When running on Vercel:
- Sets `WORKFLOW_TARGET_WORLD` to `'vercel'` if not already set
