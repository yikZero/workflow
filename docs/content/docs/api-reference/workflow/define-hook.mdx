...
```typescript
defineHook<TInput, TOutput = TInput>(
  { schema } = {},
...
<TSDoc
definition={`
interface TypedHook<TInput, TOutput = TInput> {
...
Use `TypedHook.Input<T>` to extract the input payload type from a typed hook definition.
