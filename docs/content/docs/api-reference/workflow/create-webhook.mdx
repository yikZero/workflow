...
- **Default mode** — `createWebhook(options?)` returns `Webhook<Request>`. The caller automatically receives a `202 Accepted` response (or the `Response` object you pass as `respondWith`).
- **Manual-response mode** — `createWebhook({ respondWith: 'manual' })` returns `Webhook<RequestWithResponse>`. Each request exposes a `respondWith()` method so the workflow can send a custom HTTP response from within a step function.
