...
|-----------|-------------|
| `run_started` | Workflow run was initiated |
...
| `step_retry` | Step scheduled for retry |
...
| `hook_resumed` | Hook was resumed with a payload |
| `hook_expired` | Hook expired without being resumed |

### Wait Events

| Event Type | Description |
|-----------|-------------|
| `wait_started` | Workflow entered a wait state (e.g., `sleep()`) |
