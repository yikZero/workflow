...
- A `202 Accepted` response if the webhook was created with the default mode (no `respondWith` option or a `Response` object passed to `respondWith`)
- The workflow's custom `Response` if the webhook was created with `createWebhook({ respondWith: 'manual' })` and the workflow called `request.respondWith(response)` from a step function
