...
import { createWebhook, FatalError, RetryableError, getStepMetadata, type RequestWithResponse } from "workflow";
...
async function forwardToEndpoint( // [!code highlight]
  ...
  if (response.status >= 400 && response.status < 500) {
    throw new FatalError(`Permanent rejection: ${response.status}`); // [!code highlight]
  }

  if (!response.ok) {
    // 5xx — default retry behavior
