...
export async function ingestEventsWorkflow() { // [!code highlight]
  "use workflow"; // [!code highlight]
...
<Callout type="info">
Webhooks implement `AsyncIterable`, so a single webhook can receive many requests over time. Use [`createHook()`](/docs/api-reference/workflow/create-hook) instead when you have your own ingestion layer that calls `resumeHook()` directly rather than sending HTTP requests.
</Callout>
