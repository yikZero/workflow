...
If you are starting a new agent from scratch, start with [Building Durable AI Agents](/docs/ai). Use this guide when you already have a single-connection streaming route and want the smallest migration path to reconnectable streams.
...
export async function GET(
  ...
  const tailIndex = await stream.getTailIndex(); // [!code highlight]
