...
Add the `"use workflow"` directive and wrap the streaming call inside a step function using `DurableAgent`. The step becomes a durable unit of execution that Workflow can retry and observe.
...
async function generateResponse(messages: Array<{ role: string; content: string }>) { // [!code highlight]
...
async function chatWorkflow(messages: Array<{ role: string; content: string }>) { // [!code highlight]
  "use workflow"; // [!code highlight]
  await generateResponse(messages); // [!code highlight]
} // [!code highlight]
...
The generation now runs inside a durable step. If the step fails, Workflow retries it. If the server restarts, the step resumes.
