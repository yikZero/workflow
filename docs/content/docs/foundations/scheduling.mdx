...
export async function cronDispatcherWorkflow() {
  ...
  while (true) {
    ...
    // Run cleanup every 7th iteration (weekly)
    iteration++;
...
export async function triggerDailyReport() {
  ...
  const run = await start(dailyReportWorkflow, []); // defined below // [!code highlight]
  return run.runId;
}
...
export async function triggerCleanup() {
  ...
  // stepId is stable across retries — use it if the child workflow
  // needs deduplication (e.g., as an idempotency key for external APIs)
  const run = await start(cleanupWorkflow, []); // defined below
  return run.runId;
}
...
export async function endOfMonthReportWorkflow(year: number, monthNumber: number) {
  ...
  // Sleep until the 1st of the following month.
  // monthNumber is 1-indexed (1 = Jan), and Date() months are 0-indexed,
  // so passing monthNumber directly gives us the next month.
...
export async function healthCheckWorkflow(serviceUrl: string) {
  ...
  while (true) {
    ...
    if (!status.healthy) {
      ...
      } else if (consecutiveFailures % 3 === 0) { // [!code highlight]
...
The workflow tracks `consecutiveFailures` across iterations. Every third consecutive failure triggers a service restart (at 3, 6, and 9). A single healthy check resets the counter. If failures reach ten without any recovery, a standard error permanently stops the workflow. All of this state survives server restarts because the workflow is durable.
