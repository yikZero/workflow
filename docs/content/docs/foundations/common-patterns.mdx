...
<Callout type="info">
The `start()`-inside-workflows composition pattern described below is currently available in Workflow v5 only. Keep this guidance behind the docs version switcher and do not backport it into earlier versions.
</Callout>
