...
This example stores the `runId` after the first response. For strict one-session-per-thread behavior, reserve the thread ID or conversation ID atomically before calling `start()`, then reuse the stored `runId` on retries. See [Idempotency](/docs/foundations/idempotency).
