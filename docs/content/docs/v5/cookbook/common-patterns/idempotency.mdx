...
The workflow should create and await or iterate the deterministic hook before duplicate-sensitive work. See [Run idempotency](/docs/foundations/idempotency#run-idempotency) for the full pattern, including how to steer an active run with `resumeHook()` and return `HookConflictError.conflictingRunId` after the current race between `start()` and hook registration.
