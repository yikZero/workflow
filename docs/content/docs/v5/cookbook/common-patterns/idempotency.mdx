...
The hook does not need to be resumed for this pattern. Reserving the token is enough to make one active workflow run own the idempotency key.
...
Because this pattern uses hooks for idempotency, duplicate requests can also inject additional data and steer the existing run. When the workflow awaits the hook payload, one request starts the run and later requests resume the hook with the same token. In this example, `processOrderWithConfirmation` awaits `createHook<{ confirmed: boolean }>({ token })` before processing the order:

```typescript
import { resumeHook, start } from "workflow/api";
import { HookNotFoundError } from "workflow/errors";
import { processOrderWithConfirmation } from "./workflows/process-order";
