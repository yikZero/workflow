...
Use `hook.ready` when you need to wait until the hook token has been registered without waiting for hook payload data. It resolves after `hook_created` is recorded and rejects with `HookConflictError` if another active hook already owns the same token.
...
Use `hook.ready` when the workflow needs to claim a hook token before doing other work, but does not need a payload yet:
...
async function processOrder(orderId: string) {
  ...
  try {
    ...
    await hook.ready; // [!code highlight]
...
`hook.ready` only waits for hook registration. To receive payload data from a future `resumeHook()` call, await the hook itself or iterate it with `for await...of`.
