...
<Callout type="info">
Custom hook tokens are useful for routing future events to a running workflow and for detecting token conflicts after a workflow starts. They do not prevent duplicate workflow runs from being created before the hook is registered. For one-run-per-key behavior, see [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
