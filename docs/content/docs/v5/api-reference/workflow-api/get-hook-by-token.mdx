...
<Callout type="info">
Looking up a deterministic hook token is the runtime entry point for hook-based idempotency. In a resume-or-start flow, call `getHookByToken()` first, start the workflow only when the hook is not found, then retry the lookup until the new run registers its hook. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
