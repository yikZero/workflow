...
<Callout type="info">
`getRun()` retrieves a run when you already have its `runId`. It does not look up runs by a business key. For retried requests that should reuse one workflow run, store the `runId` behind your own atomic idempotency key before calling [`start()`](/docs/api-reference/workflow-api/start). See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
