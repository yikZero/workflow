...
## Remaining Issues: Vercel BOA Deployment Tests

### Status (as of March 2026)

The V2 handler works correctly on:
- **Next.js** (Turbopack + Webpack, stable + canary) — all local dev, local prod, Postgres, and Vercel Prod tests pass
- **SvelteKit** — all tests pass
- **All Nitro-based frameworks** (Nitro, Express, Hono, Fastify, Vite, Nuxt) — local dev, local prod, and Postgres tests pass
- **Astro** — local dev, local prod, and Postgres tests pass
- **NestJS** — local dev and local prod pass

The V2 handler fails on:
- **Vercel Prod tests for Nitro-based + Astro + Example** — tests hang on the first e2e test, never producing output. The Vercel deployment is READY (serves HTTP) but workflow invocations appear to hang.

### What We Know

1. **The Vercel deployments build and deploy successfully** — the BOA builder creates the combined CJS bundle, the deployment goes to READY state.

2. **The first e2e test hangs indefinitely** — vitest starts, agent tests are skipped, but no e2e test checkmarks appear. The process is stuck.

3. **This affects ALL frameworks that use the Vercel BOA builder** — Express, Fastify, Hono, Nitro, Nuxt, Vite, Astro, Example (standalone). These all generate `.vercel/output/functions/.well-known/workflow/v1/flow.func/index.js` via `VercelBuildOutputAPIBuilder`.

4. **This does NOT affect Next.js or SvelteKit** — these have their own builders that don't use the BOA builder. They generate framework-specific route files that the framework's own bundler processes.

5. **The same BOA deployments work on main** (V1) — the V1 handler with separate `flow.func` + `step.func` works. The V2 combined `flow.func` (no `step.func`) does not.

6. **Concurrent replay overhead** — When tests DO produce output (observed in earlier runs before the hang started), we see 20+ concurrent invocations loading 150+ events across 17 pages. Each invocation loads ALL events (no incremental loading, since each invocation starts fresh). This suggests the V2 handler is being invoked many more times than necessary.

### What We Don't Know

1. **Why the first e2e test hangs** — The test calls `start()` which creates a run and queues a message. The queue should deliver the message to the `flow.func` handler. If the handler never processes the message, the test hangs forever. We need to determine:
   - Is the queue delivering the message?
   - Is the handler being invoked?
   - Is the handler crashing on startup (module initialization)?
   - Is there a CJS/ESM interop issue in the bundled output?

2. **Whether the V2 CJS bundle loads correctly** — The BOA uses `format: 'cjs'` + `bundleFinalOutput: true`. The bundle includes step registrations + workflow VM code + `workflowEntrypoint`. The `globalThis` singleton for step registry should work in CJS (it was tested locally). But the Vercel Functions runtime might handle CJS differently.

3. **Whether the stale `step.func` directory causes interference** — The BOA builder doesn't clean up the old V1 `step.func` directory. On Vercel, the build cache might preserve it. While nothing queues to `__wkf_step_*` in V2, the stale function might cause routing issues or initialization overhead.

### Debugging Plan

1. **Add a startup log to the V2 handler** — Add `console.log('V2 handler initialized')` at the top of `workflowEntrypoint()`. This will appear in Vercel function logs and confirm whether the handler loads at all.

2. **Test the BOA bundle locally** — Run the generated `flow.func/index.js` directly with Node.js to verify it loads without errors. Check that `registerStepFunction` calls execute and `workflowEntrypoint` creates a handler.

3. **Clean up stale `step.func`** — Add `rm -rf step.func` to the BOA builder's `build()` method, similar to the SvelteKit/Astro cleanup. This eliminates one variable.

4. **Check the BOA deployment's `.vc-config.json`** — Verify the queue trigger configuration matches V2 expectations (`__wkf_workflow_*` only, no `__wkf_step_*`).

5. **Deploy a minimal BOA test** — Create a simple workflow with 1 step, deploy via the standalone builder, and test manually. This isolates whether the issue is with the handler or with the test framework.

6. **Compare V1 vs V2 bundle output** — Diff the generated `flow.func/index.js` between main (V1) and v2-flow (V2) to identify structural differences that might affect Vercel Functions runtime behavior.

7. **Check Vercel Functions runtime logs** — Use Datadog to search for any function initialization errors, cold start failures, or queue message delivery issues on the BOA deployments.

8. **Test with `bundleFinalOutput: false` for BOA** — The BOA builder uses `bundleFinalOutput: true` (CJS). Try `false` to see if the separate files work better on Vercel. This would require the Vercel runtime to resolve `import` statements, which may not work for CJS.

### Priority

The Vercel BOA deployment issue is the **only remaining blocker** for the V2 PR. All other tests pass:
- 462/462 unit tests
- 5/5 world-testing embedded tests
- All local dev tests (13 frameworks)
- All local prod tests (14 configurations)
- All Postgres tests (13 frameworks)
- Next.js + SvelteKit Vercel Prod tests
