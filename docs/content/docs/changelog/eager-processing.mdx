...
In V2, the inline execution loop continues after each step. If ops are fire-and-forget (pure `waitUntil`), the loop competes for event loop time and stream data may never reach S3 before consumers try to read it. Conversely, if the loop unconditionally breaks whenever ops exist, every step with stream serialization (including all AI agent steps that pass `WritableStream` arguments) forces a queue round-trip — defeating inline execution entirely.

**Fix**: `executeStep()` awaits ops inline with a **500ms timeout**. The `flushablePipe` detects lock release via 100ms polling, so most ops settle within ~200ms. If ops settle within the timeout, `hasPendingOps` is `false` and the V2 loop continues inline. If ops don't settle (e.g., a `WritableStream` kept open across multiple step calls), `hasPendingOps` is `true`, `waitUntil` takes over, and the V2 handler breaks the loop and queues a continuation.

This means:
- **AI agent steps** (WritableStream closed after each step): ops settle inline in ~200ms, loop continues — workflow completes in 1 invocation
- **Streaming output steps** (WritableStream kept open across steps): ops don't settle, loop breaks — matches V1's per-step invocation model
- **Simple steps** (no stream ops): no timeout, no overhead — loop continues immediately
