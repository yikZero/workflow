...
`WorkflowServerWritableStream` previously buffered writes and flushed via a 10ms `setTimeout`. This caused the V2 inline loop to break on every step with `WritableStream` serialization (including all AI agent steps), since the `flushablePipe`'s `pendingOps` counter reached 0 before data actually reached the server.

**Fix**: `write()` now calls `flush()` synchronously (inline HTTP round-trip) instead of scheduling via `setTimeout`. This makes `pendingOps` accurately reflect server-side data state. The 500ms inline ops await can distinguish between:
- **Steps where ops settle** (data on server, ~200ms) → continue loop inline
- **Steps where ops don't settle** (WritableStream kept open) → break loop
