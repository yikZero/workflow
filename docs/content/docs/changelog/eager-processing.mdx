...
- **`step_started` idempotency** --- only one invocation wins the race to execute a step
...
The "losing" invocations find that the next step has already been started by a "winning" invocation, skip it (409 from `step_started`), and exit. Eventually only one invocation continues past the parallel section.
