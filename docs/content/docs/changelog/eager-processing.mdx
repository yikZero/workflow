...
**Fix**: Revert the Vercel step source map expectation to the conservative branch behavior so preview e2e only asserts source filenames where this branch actually preserves them. Concretely, `hasStepSourceMaps()` returns `false` for *every* framework on Vercel deployments — the V2 combined bundle's source-map pipeline is only known-good for `nextjs-webpack` and `sveltekit` today, and both of those already short-circuit to `false` via earlier branches in the helper. Re-applying the blanket Vercel carve-out is what keeps the 11-framework `e2e-vercel-prod` matrix green while V2 source-map coverage catches up.

**Follow-up**: Wire up consumable inline source maps in the V2 step bundle for the BOA-backed framework integrations (Astro, Express, Fastify, Hono, Nitro, Nuxt, Vite, plus the standalone `example`) and for `nextjs-turbopack`. The plan is to let each builder's `createCombinedBundle()` call carry the same esbuild source-map pipeline that `nextjs-webpack` already benefits from, and then re-introduce the per-framework matrix in `hasStepSourceMaps()` so error stacks correctly point at `99_e2e.ts` / `helpers.ts` on Vercel preview deployments. Tracking this as a deferred follow-up rather than blocking the V2 cutover, since the runtime behavior is unaffected — only the surfaced filenames in step error stacks differ.
