...
3. **Persistence** — the world backend (`world-local`, `world-vercel`, or `world-postgres`) stores each chunk as an individually addressable record that readers can consume from any index.
...
flowchart TD
    ...
    F -->|"world-postgres"| P["Rows via Drizzle ORM + LISTEN/NOTIFY"]
