...
2. **Zero-cost suspension** — when a workflow encounters a step, hook, or sleep, it records the pending operations as events, returns, and exits. No thread, connection, or process is held open.
3. **Delayed re-enqueue** — for timed waits, the queue infrastructure delivers the next message after the specified delay. The workflow doesn't exist during the gap.
