...
2. **Zero-cost suspension for external work** — when orchestration hits a step, the handler persists `step_created`, queues the step, and exits. When orchestration hits a hook, the runtime records hook state durably and exits. The workflow stays suspended until external delivery records `hook_received`. Nothing stays resident between decision points.
3. **Delayed re-enqueue for timed waits** — when orchestration hits `sleep()`, the handler persists `wait_created`, returns `timeoutSeconds`, and lets the queue schedule the next wake-up. On Vercel, waits longer than 23 hours are implemented as a chain of delayed messages.
