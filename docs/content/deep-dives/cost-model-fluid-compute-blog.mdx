...
In Workflow DevKit, that same workflow consumes compute only in a handful of brief invocations spread across seven days. Between those decision points, nothing stays alive. No worker process. No open connection. No reserved memory. The state lives in durable events and queued wake-ups, not in a running machine.
...
2. **No idle compute during suspension** — when orchestration hits a step, the handler persists `step_created`, queues the step, and exits. When orchestration hits a hook, the runtime records hook state durably and exits. The workflow stays suspended until external delivery records `hook_received`. Nothing stays resident between decision points. (Event writes and queue operations still occur at suspension boundaries, but no compute is billed while the workflow is waiting.)
...
Put rough numbers on it: an always-on worker held for 168 hours is billed for 168 hours. The queue-driven version might use a few 50 ms workflow invocations plus the actual step compute. Even before you count step time, the orchestration layer went from 604,800 seconds of wall-clock residency to well under a second of active compute.
