...
**Step bundle** → loaded by the step execution handler when a queue message arrives. The handler looks up the step ID in the registry (populated by `registerStepFunction()` calls), invokes the function with the serialized arguments from the event, and persists the result as a `step_completed` event.

**Workflow bundle** → loaded into the sandboxed VM when the workflow handler is invoked. The `WORKFLOW_USE_STEP` symbol is bound to the runtime's `useStep` proxy, and `__private_workflows` is a `Map` that the runtime reads to find the workflow function by ID. The VM environment also patches `Math.random()`, `Date.now()`, and `crypto` with deterministic alternatives — which is why step bodies can't exist here. A step that calls `crypto.randomUUID()` needs real randomness, not seeded randomness.
