...
In workflow mode, step function bodies are replaced with proxy calls through a well-known symbol:

```ts
// Workflow mode output — step body is gone
export async function createUser(email: string) {
...
The step ID (`"step//workflows/user.js//createUser"`) is derived from the file path and function name at build time — it's stable across deployments and deterministic across replays.

At runtime, `WORKFLOW_USE_STEP` is bound to the `useStep` function from the core runtime:
...
When workflow code calls `await createUser(email)`, it's actually calling through this proxy, which checks the event log for a cached result or suspends execution if the step hasn't run yet.
