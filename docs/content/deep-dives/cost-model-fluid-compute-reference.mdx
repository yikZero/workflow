...
2. **Zero-cost suspension** — when a workflow encounters a step, hook, or sleep, the handler records the pending operations as events, optionally returns a `timeoutSeconds` value, and exits. No thread, connection, or process is held open.
3. **Delayed re-enqueue** — for timed waits (`sleep()`), the queue infrastructure delivers the next message after the specified delay. The workflow process does not exist during the gap.
...
The following diagram traces a single workflow run from creation through suspension, step execution, and completion:
...
flowchart TD
    ...
    F --> G{{"Workflow suspends on step / hook / wait"}}
    G --> H["step_created / hook_created / wait_created events"]
    H --> I["Step messages queued out-of-band"]
    I --> J["Handler returns — compute released"]
    J --> K["Step handler invoked via queue"]
    K --> L["Step function executes with full Node.js access"]
    L --> M["step_completed event persisted"]
    M --> N["Workflow message re-enqueued"]
    N --> D
    G -->|"wait with resumeAt"| O["Handler returns timeoutSeconds"]
    O --> P["Queue delays next delivery"]
    P --> D
    D -->|"No pending steps/waits"| Q["Workflow runs to completion"]
