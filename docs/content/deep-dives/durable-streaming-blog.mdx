...
## Persistence: Local vs. Production

The streaming pipeline is backend-agnostic. The `Streamer` interface in `@workflow/world` defines the contract; backends implement it differently.
...
### Production (world-vercel)

The production backend in `packages/world-vercel/src/streamer.ts` delegates to the Vercel workflow-server over HTTP:
...
The production backend doesn't use an in-process event emitter — the workflow-server handles chunk storage and real-time delivery to readers over HTTP streaming responses.
...
flowchart TD
    ...
    E --> F{"World backend"}
    F -->|"Development"| G["ULID-named .bin files on disk"]
    F -->|"Production"| H["HTTP PUT to workflow-server"]
