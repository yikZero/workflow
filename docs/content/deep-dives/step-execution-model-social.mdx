...
The step runs separately with full Node.js access, charges the card via Stripe, and persists the result to an append-only event log. Then the workflow is re-enqueued.

On replay, the runtime recreates a deterministic VM sandbox — same seeded `Math.random()`, same frozen `Date.now()`, same `crypto.randomUUID()` sequence — and re-executes the workflow from the top. When it hits `await chargeCard(9900)` again, the `EventsConsumer` finds the cached `step_completed` result in the log and returns it instantly. No network call. No re-charge.
